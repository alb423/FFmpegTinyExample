...
void ring_buffer_create (struct ring_buffer *buffer, unsigned long order)
{
    ...
    if (status)
        report_exceptional_condition();
    //buffer->address = mmap (NULL, buffer->count_bytes << 1, PROT_NONE, MAP_SHARED | MAP_PRIVATE, -1, 0);
    buffer->address = mmap (NULL, buffer->count_bytes << 1, PROT_NONE, MAP_PRIVATE, file_descriptor, 0);
